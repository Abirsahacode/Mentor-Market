...
export default function StaticPage({ page }) {
  ...
  return <main className={`information-page information-${page}`}>
    <section className="simple-hero"><div className="container simple-hero-grid"><div className="simple-hero-copy"><span className="page-index"><i /> {item.label}</span><h1>{item.title}</h1><p>{item.text}</p><div className="simple-hero-actions"><Link className="button" to={isTutor ? "/register?role=tutor" : isContact ? "/login" : "/tutors"}>{isTutor ? "Start teaching" : isContact ? "Open your account" : "Explore mentors"} <ArrowRight size={16} /></Link>{!isContact && <Link className="text-link" to="/how-it-works">See how it works</Link>}</div></div><div className="information-visual"><video src={item.media} poster={item.poster} autoPlay muted loop playsInline preload="metadata" aria-label={`${item.label} preview`} /><span className="information-video-label"><i /> Mentor Market in motion</span><DemoVideo src={item.media} poster={item.poster} title={item.label} variant="icon" /><div><CirclePlay size={17} /><p>{item.note}</p></div></div></div></section>
    <section className="section container information-layout"><aside><span>What to know</span><p>Three useful details before you get started.</p><div><Sparkles size={16} /><small>Clear context creates better matches.</small></div></aside><div className="information-list">{item.items.map(([title, text], index) => <article key={title}><span>{String(index + 1).padStart(2, "0")}</span><h2>{title}</h2><p>{text}</p>{isContact && index === 0 ? <Mail size={20} /> : isContact && index === 2 ? <MapPin size={20} /> : <ArrowRight size={20} />}</article>)}</div></section>
    ...
    <section className="container information-cta"><div><small>{isTutor ? "Your experience belongs in view" : "Ready when you are"}</small><h2>{isTutor ? "Show students how you teach." : "Find a mentor who feels right."}</h2><p>{isTutor ? "Create your mentor profile and publish your first teaching service." : "Create an account or explore teaching previews first."}</p></div><Link className="button" to={isTutor ? "/register?role=tutor" : "/register"}>{isTutor ? "Become a mentor" : "Create an account"} <ArrowRight size={15} /></Link><Link className="text-link" to="/tutors">Browse mentors</Link></section>
  </main>;
}
