...
export default function TutorDetailsPage() {
  ...
  useEffect(() => {
    if (!tutor?.posts?.length) return;
    const selected = tutor.posts.find((post) => post.demo_video_url) || tutor.posts[0];
    setSelectedPostId((current) => tutor.posts.some((post) => post.id === current) ? current : selected.id);
    setBooking((current) => ({
      ...current,
  ...
  const selectedService = tutor.posts?.find((post) => post.id === selectedPostId) || featuredService;
