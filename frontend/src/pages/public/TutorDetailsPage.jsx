...
export default function TutorDetailsPage() {
  ...
  const [booking, setBooking] = useState({ class_type: "one-time", class_date: "", class_time: "", mode: "online", duration_minutes: 60 });
  ...
  return <main className="tutor-profile-page">
      ...
      <div className="container">
        ...
        <div className="profile-cinema">
          ...
          <div className="profile-summary">
            ...
            <div className="profile-cover-actions">{isStudent ? <button className="button button-ghost" type="button" onClick={save} disabled={saving}><Heart size={16} /> {saving ? "Saving…" : "Save mentor"}</button> : !user ? <Link className="button button-ghost" to="/login" state={returnState}><Heart size={16} /> Log in to save</Link> : null}<a className="button" href="#book">Request a class</a></div>
          </div>
        </div>
      </div>
    </section>
    ...
    <section className="container tutor-detail-grid"><div className="profile-panel">
      ...
      <article className="profile-section" id="reviews"><div className="profile-section-heading"><div><span className="section-mini-label">From past classes</span><h2>Student reviews</h2></div><div className="review-summary"><Star size={17} fill="currentColor" /><strong>{rating.toFixed(1)}</strong><small>{reviews.length} reviews</small></div></div>{reviews.length ? <div className="review-grid">{reviews.map((review) => <blockquote key={review.id}><span className="proof-stars">{"★".repeat(review.rating)}</span><p>“{review.comment}”</p><div><UserAvatar name={review.reviewer_name} size="tiny" /><small>{review.reviewer_name}</small></div></blockquote>)}</div> : <p className="muted">No reviews yet.</p>}</article>
    </div><aside className="booking-panel" id="book"><div className="booking-card"><div className="booking-card-head"><span>{selectedService ? "Selected class" : "Private classes from"}</span>{selectedService ? <p className="booking-selected-title">{selectedService.title}</p> : null}<h2>৳{Number(selectedService?.price || tutor.hourly_rate || 0).toLocaleString()} <small>/ session</small></h2><div><Star size={14} fill="currentColor" /> {rating.toFixed(1)} <span>·</span> {tutor.experience_years || 0} years teaching</div></div>{isStudent ? <><Alert type={feedback.includes("success") || feedback.includes("saved") ? "success" : "error"}>{feedback}</Alert><form onSubmit={book}><FormField label="Class type" name="class_type" value={booking.class_type} onChange={change} options={classTypes} required /><div className="booking-form-row"><FormField label="Date" name="class_date" type="date" min={today()} value={booking.class_date} onChange={change} required /><label className="form-field" htmlFor="class_time"><span>Time</span><select id="class_time" name="class_time" value={booking.class_time} onChange={change} required disabled={!booking.class_date || availabilityLoading}>{booking.class_date ? <option value="">{availabilityLoading ? "Loading slots…" : "Choose a time"}</option> : <option value="">Pick a date first</option>}{selectedDateSlots.map((slot) => <option value={slot.time} key={slot.label}>{slot.time}</option>)}</select></label></div><p className="booking-footnote">{booking.class_date && !availabilityLoading && !selectedDateSlots.length ? "No calendar slots are open for this date yet. Pick another day." : "Times come from the tutor’s calendar and booked sessions."}</p><FormField label="Teaching mode" name="mode" value={booking.mode} onChange={change} options={classModes} required /><button className="button button-block" disabled={submitting || !selectedPostId}><CalendarCheck2 size={17} /> {submitting ? "Sending request…" : "Request this class"}</button></form><button className="button button-block button-ghost" type="button" onClick={save} disabled={saving}><Heart size={16} /> {saving ? "Saving…" : "Save for later"}</button></> : !user ? <><p className="booking-login-copy">Log in as a student to request a trial or regular class with {firstName}.</p><Link className="button button-block" to="/login" state={returnState}>Log in to book</Link></> : <><p className="booking-login-copy">Booking is available from a student account. You are currently signed in as {user.role}.</p><Link className="button button-block" to={roleHome(user.role)}>Return to your workspace</Link></>}<ul className="trust-list"><li><ShieldCheck size={15} /><span><strong>Marketplace safeguards</strong><small>Admin moderation and reporting tools</small></span></li><li><CalendarCheck2 size={15} /><span><strong>Schedule with context</strong><small>Manage changes from your dashboard</small></span></li><li><Check size={15} /><span><strong>Mock payment only</strong><small>No real payment is processed</small></span></li></ul><p className="booking-footnote"><Sparkles size={13} /> You are requesting a time, not confirming a charge.</p></div></aside></section>
    <div className="profile-mobile-cta"><span><small>{selectedService ? "Selected class" : "Private class"}</small><strong>৳{Number(selectedService?.price || tutor.hourly_rate || 0).toLocaleString()} / session</strong></span><a className="button" href="#book">Request a class</a></div>
  </main>;
}
