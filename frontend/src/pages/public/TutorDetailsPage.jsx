...
export default function TutorDetailsPage() {
  ...
  const handleJoinWaitlist = async (slotTime) => {
    setFeedback("");
    ...
    try {
      ...
      setFeedback(`Joined waitlist for ${booking.class_date} at ${slotTime} successfully!`);
    } catch (err) {
  ...
  const openDateSlots = selectedDateSlots.filter((slot) => !slot.is_booked);
  const bookedDateSlots = selectedDateSlots.filter((slot) => slot.is_booked);
  ...
  const availabilityFieldError = availabilityIsResolved
    ? availabilityError || (!openDateSlots.length ? (bookedDateSlots.length ? "All slots for this date are booked. Join the waitlist below!" : "No open calendar slots are available for this date. Pick another day.") : "")
    : "";
