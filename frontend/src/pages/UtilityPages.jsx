...
export function NotificationsPage() {
  ...
  const [action, setAction] = useState({ id: null, status: "", error: "" });
  const announceUpdate = () => window.dispatchEvent(new CustomEvent("mentor-market:notifications-updated"));
  const read = async (id) => {
    setAction({ id, status: "pending", error: "" });
    try { await api.patch(`/notifications/${id}/read`); await reload(); announceUpdate(); setAction({ id: null, status: "success", error: "" }); }
    catch (requestError) { setAction({ id, status: "error", error: getErrorMessage(requestError) }); }
  };
  const readAll = async () => {
    setAction({ id: "all", status: "pending", error: "" });
    try { await api.patch("/notifications/read-all"); await reload(); announceUpdate(); setAction({ id: null, status: "success", error: "" }); }
    catch (requestError) { setAction({ id: "all", status: "error", error: getErrorMessage(requestError) }); }
  };
  const actionPending = action.status === "pending";
  return <section className="notifications-page"><PageHeader eyebrow="Activity centre" title="What changed while you were learning" description="A clean timeline of applications, bookings, learning tasks, messages, payments, and verification." actions={<button className="button button-ghost" onClick={readAll} disabled={actionPending || !data.some((item) => !item.is_read)} aria-busy={action.id === "all" && actionPending || undefined}><CheckCheck size={16} /> {action.id === "all" && actionPending ? "Marking read…" : "Mark all read"}</button>} /><Alert>{action.error || error}</Alert>{loading ? <LoadingSpinner /> : data.length ? <div className="notification-list">{data.map((item) => { const Icon = notificationIcon(item.type); const itemPending = action.id === item.id && actionPending; return <article className={item.is_read ? "notification-item" : "notification-item unread"} key={item.id}><span><Icon size={17} /></span><div><div className="notification-title-line"><strong>{item.title}</strong>{!item.is_read && <i>New</i>}</div><p>{item.message}</p><small>{new Date(item.created_at).toLocaleString([], { dateStyle: "medium", timeStyle: "short" })}</small></div>{!item.is_read && <button className="button button-tiny button-ghost" disabled={actionPending} aria-busy={itemPending || undefined} onClick={() => read(item.id)}>{itemPending ? "Updating…" : "Mark read"}</button>}</article>; })}</div> : <EmptyState icon={Bell} title="You’re all caught up" text="New marketplace and learning activity will land here." />}</section>;
}

export function ReportsPage() {
  const { data, loading, error, reload } = useApi("/reports");
  const { data: conversations, error: conversationsError } = useApi("/messages/conversations");
