...
function UserDashboard({ role }) {
  ...
  return <>
    ...
    <div className="dashboard-two-column dashboard-user-columns">
      <article className="panel schedule-panel"><div className="panel-heading"><div><span className="panel-eyebrow">Schedule</span><h2>Upcoming classes</h2></div><Link to={`/${role}/bookings`}>View all <ArrowRight size={14} /></Link></div>{upcoming.length ? upcoming.slice(0, 5).map((booking) => <div className="activity-row" key={booking.id}><span className="activity-date"><strong>{new Date(booking.class_date).getDate()}</strong><small>{new Date(booking.class_date).toLocaleString("en", { month: "short" })}</small></span><UserAvatar name={role === "student" ? booking.tutor_name : booking.student_name} size="tiny" /><div><strong>{role === "student" ? booking.tutor_name : booking.student_name}</strong><p><Clock3 size={13} /> {booking.class_time?.slice(0, 5)} · {booking.class_type} · {booking.mode}</p></div><span className={`status-badge status-${booking.status}`}>{booking.status}</span></div>) : <div className="calm-empty"><CalendarDays size={23} /><strong>No upcoming classes</strong><p>Booked classes will appear here.</p></div>}</article>
