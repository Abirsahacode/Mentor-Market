...
export function PaymentsPage() {
  ...
  const [form, setForm] = useState({ booking_id: "", amount: "", payment_method: "bKash" });
  const [message, setMessage] = useState("");
  const change = (event) => setForm((current) => ({ ...current, [event.target.name]: event.target.value }));
  const create = async (event) => { event.preventDefault(); try { await api.post("/payments", form); setMessage("Mock payment created. Mark it paid when ready."); reload(); } catch (requestError) { setMessage(getErrorMessage(requestError)); } };
  ...
  return <section className="finance-page"><PageHeader eyebrow="Demo wallet" title="Payments with a clear paper trail" description="Create and complete mock transactions safely. No real payment provider or money is contacted." /><div className="finance-notice"><WalletCards size={20} /><div><strong>Simulation mode</strong><p>Every action is stored in the project database so the workflow can be tested end to end.</p></div></div><div className="dashboard-grid finance-studio-grid"><form className="panel" onSubmit={create}><div className="panel-heading"><div><span className="panel-eyebrow">New transaction</span><h2>Create mock payment</h2></div><Banknote size={20} /></div><Alert type={message.includes("created") ? "success" : "error"}>{message}</Alert><FormField name="booking_id" label="Booking" value={form.booking_id} onChange={change} options={(bookings || []).filter((booking) => !data.some((payment) => payment.booking_id === booking.id)).map((booking) => ({ value: booking.id, label: `#${booking.id} · ${booking.tutor_name}` }))} required /><FormField name="amount" label="Amount (৳)" type="number" min="1" value={form.amount} onChange={change} required /><FormField name="payment_method" label="Payment method" value={form.payment_method} onChange={change} options={["card", "bKash", "Nagad", "Rocket", "cash"]} /><button className="button">Create payment</button></form><article className="panel panel-wide"><div className="panel-heading"><div><span className="panel-eyebrow">Ledger</span><h2>Payment history</h2></div><ReceiptText size={20} /></div><Alert>{error}</Alert>{loading ? <LoadingSpinner /> : data.length ? <DataTable label="Payment history" rows={data} columns={columns} actions={(row) => row.status === "pending" && <button className="button button-tiny" onClick={() => markPaid(row.id)}>Mark paid</button>} /> : <EmptyState icon={ReceiptText} title="No transactions yet" text="Create a mock payment after booking a class." />}</article></div></section>;
}
