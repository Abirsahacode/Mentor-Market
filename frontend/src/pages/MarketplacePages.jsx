...
export function MyListingsPage({ type }) {
  ...
  const columns = isRequest ? [
    ...
    { key: "teaching_mode", label: "Mode" }, { key: "views", label: "Views" }, { key: "saves", label: "Saves" }, { key: "bookings", label: "Bookings" },
    { key: "status", label: "Status", render: status },
  ];
  const actions = (row, reload) => <>{!isRequest && row.status === "draft" && <button className="button button-tiny" onClick={async () => { await api.patch(`/tutor-posts/${row.id}`, { status: "active" }); reload(); }}>Publish</button>}<button className="button button-tiny button-ghost" onClick={async () => { await api.patch(`${isRequest ? "/student-requests" : "/tutor-posts"}/${row.id}`, { status: isRequest ? "closed" : row.status === "active" ? "inactive" : "active" }); reload(); }}>{isRequest ? "Close" : row.status === "active" ? "Pause" : "Activate"}</button>{!isRequest && <Link className="button button-tiny button-ghost" to={`/tutor/services/${row.id}/curriculum`}>Learning path</Link>}{!isRequest && <button className="button button-tiny button-ghost" onClick={async () => { await api.post("/tutor-posts", { title: `Copy of ${row.title}`, subject: row.subject, level: row.level, price: row.price, teaching_mode: row.teaching_mode, location: row.location, availability: row.availability, has_trial: row.has_trial, thumbnail_url: row.thumbnail_url, demo_video_url: row.demo_video_url, description: row.description, status: "draft" }); reload(); }}>Duplicate</button>}<button className="button button-tiny button-danger" onClick={async () => { if (window.confirm("Delete this item?")) { await api.delete(`${isRequest ? "/student-requests" : "/tutor-posts"}/${row.id}`); reload(); } }}>Delete</button></>;
