...
export default function DemoVideo({ src, poster, title = "Tutor demo lesson", label = "Watch demo", variant = "button" }) {
    ...
    {open && createPortal(
      <div className="media-modal" role="dialog" aria-modal="true" aria-label={title} onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
        <div ref={dialogRef} className="media-dialog" tabIndex="-1">
          ...
          <video controls autoPlay playsInline poster={poster} src={src}>Your browser does not support embedded video.</video>
