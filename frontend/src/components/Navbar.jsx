  ...
  return (
    <><header className={`navbar ${scrollState.elevated ? "is-elevated" : ""}`} style={{ "--nav-progress": `${scrollState.progress}%` }}>
      <div className="container nav-inner">
        <div className="nav-brand-lockup"><Brand onClick={close} /></div>
        ...
        <nav ref={navRef} id="public-navigation" className={open ? "nav-links is-open" : "nav-links"} aria-label="Primary navigation">
          <div className="nav-primary">
            ...
            <NavLink to="/student-requests" onClick={close}>Student briefs</NavLink>
            <NavLink to="/how-it-works" onClick={close}>How it works</NavLink>
            <NavLink className="nav-teach" to="/become-a-tutor" onClick={close}>Become a mentor</NavLink>
          </div>
          <div className="nav-actions">
            {user ? <><Link className="button button-small" to={roleHome(user.role)} onClick={close}><LayoutDashboard size={15} /> Workspace</Link><button className="nav-logout" onClick={handleLogout}><LogOut size={15} /> Log out</button></> : <><Link className="nav-login" to="/login" onClick={close}>Log in</Link><Link className="button button-small" to="/register" onClick={close}>Join free <ArrowRight size={15} /></Link></>}
          </div>
        </nav>
      </div>
