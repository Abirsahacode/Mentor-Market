...
const TutorProfile = {
  ...
  async search({ subject, location, mode, maxPrice, minRating, q, sort, limit, offset }) {
    ...
    if (q) { clauses.push("(u.full_name LIKE ? OR tp.bio LIKE ? OR tp.qualifications LIKE ? OR CAST(tp.subjects AS CHAR) LIKE ? OR fp.title LIKE ? OR fp.subject LIKE ?)"); values.push(`%${q}%`, `%${q}%`, `%${q}%`, `%${q}%`, `%${q}%`, `%${q}%`); }
    ...
    }[sort] || "tp.is_verified DESC, tp.average_rating DESC, tp.experience_years DESC";
    const [rows] = await db.query(
