...
export function SavedTutorsPage() {
  const { data, loading, error, reload } = useApi("/students/saved-tutors");
  const remove = async (id) => { await api.delete(`/students/saved-tutors/${id}`); reload(); };
  return <section className="dashboard-marketplace-page"><PageHeader eyebrow="Your shortlist" title="Saved tutors" description="A focused set of mentors to revisit, compare, and contact when you are ready." /><Alert>{error}</Alert>{loading ? <LoadingSpinner /> : data.length ? <div className="card-grid">{data.map((tutor) => <TutorCard tutor={tutor} key={tutor.user_id} onSave={remove} />)}</div> : <EmptyState icon={BookMarked} title="No saved tutors" text="Tap the heart on a tutor profile to create your shortlist." />}</section>;
}
...
export function ApplicationsPage() {
  ...
  const decide = async (id, nextStatus) => { await api.patch(`/applications/${id}/status`, { status: nextStatus }); reload(); };
  const columns = [
  ...
  const actions = user.role === "student" ? (row) => row.status === "pending" ? <><button className="button button-tiny" onClick={() => decide(row.id, "accepted")}>Accept</button><button className="button button-tiny button-danger" onClick={() => decide(row.id, "rejected")}>Reject</button></> : null : undefined;
  return <section><PageHeader eyebrow="Proposal desk" title={user.role === "student" ? "Applications received" : "Applications sent"} description={user.role === "student" ? "Compare approaches, availability, and price before choosing the best fit." : "Track every proposal and see where each conversation stands."} /><Alert>{error}</Alert>{loading ? <LoadingSpinner /> : data.length ? <DataTable label="Applications" rows={data} columns={columns} actions={actions} /> : <EmptyState icon={FileCheck2} title="No applications yet" text="Proposals will appear here as the marketplace responds." />}</section>;
}

export function BookingsPage() {
  const { user } = useAuth();
  const { data, loading, error, reload } = useApi("/bookings");
  const update = async (id, nextStatus) => { await api.patch(`/bookings/${id}`, { status: nextStatus }); reload(); };
  const columns = [
  ...
  const actions = (row) => <>{user.role === "tutor" && row.status === "pending" && <button className="button button-tiny" onClick={() => update(row.id, "confirmed")}>Confirm</button>}{user.role === "tutor" && row.status === "confirmed" && <button className="button button-tiny" onClick={() => update(row.id, "completed")}>Complete</button>}{!["completed", "cancelled"].includes(row.status) && <button className="button button-tiny button-danger" onClick={() => update(row.id, "cancelled")}>Cancel</button>}</>;
  return <section><PageHeader eyebrow="Class calendar" title="My bookings" description="Manage upcoming classes, confirmations, and the sessions already completed." /><Alert>{error}</Alert>{loading ? <LoadingSpinner /> : data.length ? <DataTable label="Bookings" rows={data} columns={columns} actions={actions} /> : <EmptyState icon={CalendarDays} title="No classes booked" text="Your class schedule will appear here after a booking is created." />}</section>;
}
